...
import com.jackdaw.jinjobbackenduserservice.mapper.AppUserInfoMapper;
...
public class AppUserInfoServiceImpl implements AppUserInfoService {
     ...
     */
    @Override
    public PaginationResultVO<AppUserInfoDto> findDtoListByPage(AppUserInfoQuery param) {
        int count = this.findCountByParam(param);
        int pageSize = param.getPageSize() == null ? PageSize.SIZE15.getSize() : param.getPageSize();
        ...
        param.setSimplePage(page);
        List<AppUserInfo> list = this.findListByParam(param);
        List<AppUserInfoDto> dtoList = new ArrayList<AppUserInfoDto>();
        for (AppUserInfo appUserInfo:
             list) {
            AppUserInfoDto appUserLoginDto = new AppUserInfoDto();
            BeanUtil.copyProperties(appUserLoginDto,appUserInfo);
            dtoList.add(appUserLoginDto);
        }
        PaginationResultVO<AppUserInfoDto> result = new PaginationResultVO(count, page.getPageSize(), page.getPageNo(), page.getPageTotal(), dtoList);
        return result;
    }
    ...
    public void register(AppUserInfo appUserInfo) {
        ...
        if (null != userInfo) {
            throw new BusinessException(ErrorCode.API_REQUEST_ERROR,"邮箱账号已经存在");
        }
        ...
        if (count > 0) {
            throw new BusinessException(ErrorCode.API_REQUEST_ERROR,"昵称已经存在");
        }
    ...
    public String login(String email, String password, String ip) {
        ...
        if (null == userInfo) {
            throw new BusinessException(ErrorCode.API_REQUEST_ERROR,"账号或者密码错误");
        }
        ...
        if (!UserStatusEnum.ENABLE.getStatus().equals(userInfo.getStatus())) {
            throw new BusinessException(ErrorCode.API_REQUEST_ERROR,"账号已经禁用");
        }
        ...
        loginDto.setExamAcScore(userInfo.getExamAcScore());
        loginDto.setExamAcSum(userInfo.getExamAcSum());
        loginDto.setOjAcSum(userInfo.getOjAcSum());
        loginDto.setBackground(userInfo.getBackground());
        loginDto.setTags(userInfo.getTags());
        loginDto.setDescription(userInfo.getDescription());
    ...
    @Override
    public String getCheckinInfo(String userId) {
        ...
        return redisUtils.getBit(redisKey, dayOfMonth);
    }
}