...
import com.jackdaw.jinjobbackendquestionservice.mapper.CategoryMapper;
import com.jackdaw.jinjobbackendquestionservice.mapper.ExamQuestionMapper;
...
public class CategoryServiceImpl implements CategoryService {
    ...
    public Integer deleteCategoryByCategoryId(Integer categoryId) {
        ...
        if (count > 0) {
            throw new BusinessException(ErrorCode.API_REQUEST_ERROR,"问题库正在使用改分类无法删除");
        }
        ...
        if (count > 0) {
            throw new BusinessException(ErrorCode.API_REQUEST_ERROR,"试题库正在使用改分类无法删除");
        }
    ...
    public void saveCategory(Category category) {
        ...
        Integer count = this.categoryMapper.selectCount(categoryQuery);
        if (count > 1) {
            throw new BusinessException(ErrorCode.API_REQUEST_ERROR,"分类名称重复");
        }
        if (null == category.getCategoryId()) {
            return;
        }
