...
import com.jackdaw.jinjobbackendquestionservice.mapper.ShareInfoMapper;
...
public class ShareInfoServiceImpl implements ShareInfoService {
    ...
    public void saveShare(ShareInfo shareInfo) {
        if (null == shareInfo.getShareId()) {
            ...
            ShareInfo dbInfo = this.shareInfoMapper.selectByShareId(shareInfo.getShareId());
            //只能自己发布的才能修改
            if (!dbInfo.getCreateUserId().equals(shareInfo.getCreateUserId())) {
                throw new BusinessException(ErrorCode.PARAMS_ERROR.getCode(),ErrorCode.PARAMS_ERROR.getMessage());
            }
            shareInfo.setCreateUserId(null);
            shareInfo.setCreateUserName(null);
    ...
    @Override
    public void delShareBatch(String shareIds, String userId) {
        String[] shareIdArray = shareIds.split(",");
        if (userId != null) {
            ShareInfoQuery infoQuery = new ShareInfoQuery();
            infoQuery.setShareIds(shareIdArray);
    ...
    @Override
    public ShareInfo showShareDetailNext(ShareInfoQuery query, Integer currentId, Boolean updateReadCount) {
